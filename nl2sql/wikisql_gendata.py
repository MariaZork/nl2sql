...
def basic_tokenize(doc):
    ...
    for pos, c in enumerate(doc):
        if is_whitespace(c):
            ...
            if prev_is_whitespace or is_punctuation(c) or prev_is_punc or (
                    prev_is_num and not str(c).isnumeric()):
                doc_tokens.append(c)
...
class SQLExample(object):
            ...
            if conditions is not None and len(conditions) > 0:
                ...
                for cond in conditions:
                    ...
                    for i in range(len(self.tokens)):
                        if " ".join(self.tokens[i:i + val_len]).lower(
                        ) != " ".join(value_tokens).lower():
                            continue
                        s = self.word_to_char_start[i]
                        e = len(question) if i + val_len >= len(
                            self.word_to_char_start
                        ) else self.word_to_char_start[i + val_len]
                    ...
                    if cur_start is None:
                        ...
                        self.value_start_end[value] = (cur_start,
                                                       cur_start + val_len)
        else:
    ...
    def load_from_json(s):
        d = json.loads(s)
        keys = [
            "qid", "question", "table_id", "column_meta", "agg", "select",
            "conditions", "tokens", "char_to_word", "word_to_char_start",
            "value_start_end", "valid"
        ]
    ...
    for phase in ["train", "dev", "test"]:
        ...
        with open(output_file, "w", encoding="utf8") as f:
            for raw_sample in utils.read_jsonl(src_file):
                ...
                cond_col_values = {
                    header[cond[0]]: str(cond[2])
                    for cond in sql["conds"]
                }
                column_meta = []
                for col in header:
                    if col in cond_col_values:
                        column_meta.append((col, colTypes[table_id][col],
                                            cond_col_values[col]))
                    else:
                        detected_val = None
                ...
                example = SQLExample(cnt, raw_sample["question"], table_id,
                                     column_meta, sql["agg"], int(sql["sel"]),
