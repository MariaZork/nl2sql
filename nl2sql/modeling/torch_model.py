...
class HydraTorch(BaseModel):
    ...
    def train_on_batch(self, batch):
        if self.optimizer is None:
            ...
            self.model, self.optimizer, self.scheduler, batch = self.accelerator.prepare(
                self.model,
                self.optimizer,
                self.scheduler,
                batch,
    ...
    def model_inference(self, model_inputs):
        ...
        model_outputs = {}
        batch_size = 512
        model_inputs = self.accelerator.prepare(model_inputs,
                                                device_placement=[True])

        for start_idx in range(0, model_inputs["input_ids"].shape[0],
                               batch_size):
            input_tensor = {
                k: model_inputs[k][start_idx:start_idx + batch_size].to(
                    self.device)
...
class HydraNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        drop_rate = float(
            config["drop_rate"]) if "drop_rate" in config else 0.0
        ...
        self.where_num = nn.Linear(bert_hid_size,
                                   int(config["where_column_num"]) + 1)
    ...
    def forward(self,
        ...
        # value_span_mask[:, 0] = 1
        start_logit = start_end_logit[:, :,
                                      0] * value_span_mask - 1000000.0 * (
                                          1 - value_span_mask)
        end_logit = start_end_logit[:, :, 1] * value_span_mask - 1000000.0 * (
            1 - value_span_mask)
        ...
        if select is not None:
            ...
            loss += bceloss(column_func_logit[:, 2],
                            (1 - select.float()) * (1 - where.float()))
...
if __name__ == "__main__":
    ...
    inputs = tokenizer.encode_plus("Here is some text to encode",
