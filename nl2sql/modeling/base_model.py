...
class BaseModel(object):
    ...
    def dataset_inference(self, dataset: SQLDataset):
        ...
        print("model prediction end, time elapse: {0}".format(time.time() -
                                                              start_time))
    ...
    def predict_SQL(self, dataset: SQLDataset, model_outputs=None):
        ...
        sqls = []
        for input_feature, model_output in zip(dataset.input_features,
                                               model_outputs):
            agg, select, where, conditions = self.parse_output(
                input_feature, model_output, [])

            conditions_with_value_texts = []
            for wc in where:
                _, op, vs, ve = conditions[wc]
                word_start, word_end = input_feature.subword_to_word[wc][
                    vs], input_feature.subword_to_word[wc][ve]
                ...
                value_span_text = input_feature.question[
                    char_start:char_end].rstrip()
    ...
    def predict_SQL_with_EG(self,
        ...
        for input_feature, model_output in zip(dataset.input_features,
                                               model_outputs):
            agg, select, where_num, conditions = self.beam_parse_output(
                input_feature, model_output, beam_size)
            ...
            for condition in conditions:
                ...
                result, sql = engine.execute_dict_query(
                    input_feature.table_id, query)
    ...
    def _get_where_num(self, output):
        ...
        where_num_scores = np.average(output["where_num"],
                                      axis=0,
                                      weights=relevant_prob)
    ...
    def parse_output(self, input_feature, model_output, where_label=[]):
        ...
        def get_span(i):
            ...
            value_start, value_end = model_output["value_start"][
                i,
                segment_ids == 1], model_output["value_end"][i,
                                                             segment_ids == 1]
            l = len(value_start)
            sum_mat = value_start.reshape((l, 1)) + value_end.reshape((1, l))
            span = (0, 0)
            for cur_span, _ in sorted(np.ndenumerate(sum_mat),
                                      key=lambda x: x[1],
                                      reverse=True):
                if cur_span[1] < cur_span[0] or cur_span[
                        0] == l - 1 or cur_span[1] == l - 1:
                    continue
                span = cur_span
                break
        ...
        select_id_prob = sorted(enumerate(model_output["column_func"][:, 0]),
                                key=lambda x: x[1],
                                reverse=True)
        select = select_id_prob[0][0]
        agg = np.argmax(model_output["agg"][select, :])

        where_id_prob = sorted(enumerate(model_output["column_func"][:, 1]),
                               key=lambda x: x[1],
                               reverse=True)
    ...
    def beam_parse_output(self, input_feature, model_output, beam_size=5):
        ...
        def get_span(i):
            ...
            sum_mat = value_start.reshape((l, 1)) + value_end.reshape((1, l))
            spans = []
            for cur_span, sum_logp in sorted(np.ndenumerate(sum_mat),
                                             key=lambda x: x[1],
                                             reverse=True):
                if cur_span[1] < cur_span[0] or cur_span[
                        0] == l - 1 or cur_span[1] == l - 1:
                    continue
                spans.append(
                    (cur_span[0] + offset, cur_span[1] + offset, sum_logp))
