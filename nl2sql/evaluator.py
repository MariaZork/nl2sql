...
class HydraEvaluator():
    ...
    def _eval_imp(self, eval_data: SQLDataset, get_sq=True):
        ...
        for input_feature, model_output in zip(eval_data.input_features,
                                               model_outputs):
            ...
            agg, select, where, conditions = self.model.parse_output(
                input_feature, model_output, wc_label)
            if agg != agg_label:
                cur_acc["agg"] = 0
            ...
            for w in wc_label:
                ...
                if vs != input_feature.value_start[
                        w] or ve != input_feature.value_end[w]:
                    cur_acc["val"] = 0
            ...
            if ("DEBUG" in self.config or get_sq) and not all_correct:
                try:
                    ...
                    pred_sq = input_feature.output_SQ(
                        agg=agg,
                        sel=select,
                        conditions=[conditions[w] for w in where])
                    task_cor_text = "".join(
                        [str(cur_acc[k]) for k in items if k in cur_acc])
                    sq.append([
                        str(cnt), input_feature.question,
        ...
        for item in items:
            result_str.append(item +
                              ":{0:.1f}".format(acc[item] * 100.0 / cnt))
