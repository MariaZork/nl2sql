...
class InputFeature(object):
    ...
    def __init__(self, question, table_id, tokens, word_to_char_start,
                 word_to_subword, subword_to_word, input_ids, input_mask,
                 segment_ids):
    ...
    def output_SQ(self, agg=None, sel=None, conditions=None, return_str=True):
        ...
        if agg is None and sel is None and conditions is None:
            ...
            for i in range(len(self.where)):
                if self.where[i] == 0:
                    continue
                conditions.append(
                    (i, self.op[i], self.value_start[i], self.value_end[i]))
        ...
        for wc, op, vs, ve in conditions:
            ...
            op_text = cond_ops[op]
            word_start, word_end = self.subword_to_word[wc][
                vs], self.subword_to_word[wc][ve]
            char_start = self.word_to_char_start[word_start]
            char_end = len(self.question) if word_end + 1 >= len(
                self.word_to_char_start) else self.word_to_char_start[word_end
                                                                      + 1]
            value_span_text = self.question[char_start:char_end]
            cond_texts.append(column_text + op_text + value_span_text.rstrip())

        if return_str:
            sq = agg_text + ", " + select_text + ", " + " AND ".join(
                cond_texts)
        else:
...
class HydraFeaturizer(object):
    ...
    def __init__(self, config):
        ...
        self.colType2token = {"string": "[unused1]", "real": "[unused2]"}

    def get_input_feature(self, example: SQLExample, config):
        max_total_length = int(config["max_total_length"])

        input_feature = InputFeature(example.question, example.table_id, [],
        ...
        for column, col_type, _ in example.column_meta:
            ...
            word_to_subword = [(pos[0] + column_token_length,
                                pos[1] + column_token_length)
                               for pos in word_to_subword]
    ...
    def fill_label_feature(self, example: SQLExample,
                           input_feature: InputFeature, config):
        max_total_length = int(config["max_total_length"])
        ...
        for colidx, column_meta in enumerate(example.column_meta):
            ...
            try:
                ...
                assert s < max_total_length and input_feature.input_mask[
                    colidx][s] == 1
                assert e < max_total_length and input_feature.input_mask[
                    colidx][e] == 1
    ...
    def load_data(self, data_paths, config, include_label=False):
        ...
        if include_label:
            for k in [
                    "agg", "select", "where_num", "where", "op", "value_start",
                    "value_end"
            ]:
                model_inputs[k] = []
        ...
        for data_path in data_paths.split("|"):
            ...
            for line in open(data_path, encoding="utf8"):
                ...
                if include_label:
                    success = self.fill_label_feature(example, input_feature,
                                                      config)
                    if not success:
                        continue
                ...
                if include_label:
                    ...
                    model_inputs["value_start"].extend(
                        input_feature.value_start)
...
class SQLDataset(torch_data.Dataset):
    ...
    def __init__(self, data_paths, config, featurizer, include_label=False):
        ...
        self.input_features, self.model_inputs, self.pos = self.featurizer.load_data(
            data_paths, config, include_label)
...
if __name__ == "__main__":
    ...
    train_data = SQLDataset(config["train_data_path"], config, featurizer,
                            True)
