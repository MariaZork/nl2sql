...
parser.add_argument("--output_path",
                    type=str,
                    default="output",
                    help="folder path for all outputs")
parser.add_argument(
    "--model_path",
...
if args.job == "train":
    ...
    config["num_train_steps"] = int(num_samples * int(config["epochs"]) /
                                    int(config["batch_size"]))
    step_per_epoch = num_samples / int(config["batch_size"])
    print("total_steps: {0}, warm_up_steps: {1}".format(
    ...
    while True:
        for batch_id, batch in enumerate(train_data_loader):
            ...
            if batch_id % 100 == 0:
                ...
                print("[{3}] epoch {0}, batch {1}, batch_loss={2:.4f}".format(
                    epoch, batch_id, cur_loss,
