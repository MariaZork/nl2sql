...
def create_base_model(config):
    print(os.getcwd())
    weights_name = pretrained_weights[(config["base_class"],
                                       config["base_name"])]
    if config["base_class"] == "bert":
        return transformers.BertModel.from_pretrained("../models/bert-base")
    elif config["base_class"] == "roberta":
        return transformers.RobertaModel.from_pretrained(
            "../models/roberta-base")
    elif config["base_class"] == "albert":
        return transformers.AlbertModel.from_pretrained(weights_name)
    else:
        raise Exception("base_class {0} not supported".format(
            config["base_class"]))
...
if __name__ == "__main__":
    qtokens = [
        'Tell', 'me', 'what', 'the', 'notes', 'are', 'for', 'South',
        'Australia'
    ]
